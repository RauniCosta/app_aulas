...
final turmaObjetoSelecionadaProvider = StateProvider.autoDispose<TurmaModel?>(
  (ref) => null,
); // AGORA É O OBJETO TURMA
final ucSelecionadaProvider = StateProvider.autoDispose<String?>((ref) => null);
final diaDaSemanaProvider = StateProvider.autoDispose<String>(
  (ref) => 'Segunda-feira',
);
final turnoSelecionadoProvider = StateProvider.autoDispose<String>(
  (ref) => 'Manhã (08:00 - 12:30)',
);
final docentesSelecionadosProvider = StateProvider.autoDispose<Set<String>>(
  (ref) => {},
);
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text(
            'Atribua disciplinas e horários aos professores e turmas cadastradas',
                        ...
                        children: [
                            ...
                            data: (cursos) =>
                                DropdownButtonFormField<CursoModel>(
                                  ...
                                  onChanged: (val) {
                                    ...
                                    ref
                                            .read(
                                              ucSelecionadaProvider.notifier,
                                            )
                                            .state =
                                        null; // Reseta UC
                                  },
                          ...
                          if (cursoObjeto == null)
                            const TextField(
                              ...
                              decoration: InputDecoration(
                                labelText:
                                    'Selecione um curso primeiro para ver as Turmas',
                                border: OutlineInputBorder(),
                              ),
                            )
                          else
                            Consumer(
                              builder: (context, ref, child) {
                                ...
                                return turmasAsync.when(
                                  ...
                                  error: (e, s) =>
                                      Text('Erro ao carregar Turmas: $e'),
                                  data: (turmas) {
                                    if (turmas.isEmpty) {
                                      return const Text(
                            ...
                            Consumer(
                              builder: (context, ref, child) {
                                final ucsAsync = ref.watch(
                                  ucsPorCursoProvider(cursoObjeto.id),
                                );
                                ...
                                return ucsAsync.when(
                                  ...
                                  data: (ucs) {
                                    ...
                                    return DropdownButtonFormField<String>(
                                      decoration: const InputDecoration(
                                        labelText:
                                            'Unidade Curricular (UC) - Ordem Alfabética',
                                        border: OutlineInputBorder(),
                                      ),
                                      value: ucSelecionada,
                                      items: ucs.map((uc) {
                          ...
                          Expanded(
                            child: docentesAsync.when(
                              loading: () => const CircularProgressIndicator(),
                              error: (e, s) =>
                                  const Text('Erro ao carregar docentes'),
                              data: (docentes) => ListView.builder(
                                itemCount: docentes.length,
                                itemBuilder: (context, index) {
                                  final docente = docentes[index];
                                  final isSelected = docentesSelecionados
                                      .contains(docente.nome);
                                  return CheckboxListTile(
                                    dense: true,
                                    title: Text(docente.nome),
                                    subtitle: Text(
                                      'Disponível: ${docente.diasEscala.join(", ")}',
                                    ),
                                    value: isSelected,
                                    activeColor: const Color(0xFF2E8B57),
                                    onChanged: (bool? checked) {
                                      final setNotifier = ref.read(
                                        docentesSelecionadosProvider.notifier,
                                      );
                                      if (checked == true) {
                                        setNotifier.state = {
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _buildResumoItem(
                            'Curso:',
                            cursoObjeto?.nome ?? 'Não selecionado',
                          ),
                          _buildResumoItem(
                            'Turma:',
                            turmaObjeto != null
                                ? '${turmaObjeto.nome} (${turmaObjeto.periodo})'
                                : 'Não selecionada',
                          ),
                          _buildResumoItem(
                            'Datas:',
                            turmaObjeto != null
                                ? '${turmaObjeto.dataInicio.day}/${turmaObjeto.dataInicio.month} até ${turmaObjeto.dataFim.day}/${turmaObjeto.dataFim.month}/${turmaObjeto.dataFim.year}'
                                : 'A definir',
                          ),
                          _buildResumoItem(
                            'Disciplina (UC):',
                            ucSelecionada ?? 'Não selecionada',
                          ),
                          _buildResumoItem(
                            'Dia e Turno:',
                            '$diaDaSemana | $turnoSelecionado',
                          ),
                          _buildResumoItem(
                            'Professores:',
                            '${docentesSelecionados.length} selecionado(s)',
                          ),
                          ...
                          SizedBox(
                            ...
                            child: ElevatedButton(
                              onPressed: () async {
                                // 1. Lemos todos os valores que o coordenador selecionou na tela
                                final curso = ref.read(
                                  cursoObjetoSelecionadoProvider,
                                );
                                final turma = ref.read(
                                  turmaObjetoSelecionadaProvider,
                                );
                                final uc = ref.read(ucSelecionadaProvider);
                                final docentes = ref.read(
                                  docentesSelecionadosProvider,
                                );
                                final dia = ref.read(diaDaSemanaProvider);
                                final turno = ref.read(
                                  turnoSelecionadoProvider,
                                );
                                ...
                                try {
                                  final repo = ref.read(
                                    escalaRepositoryProvider,
                                  );

                                  // NOVA LÓGICA: Criamos a escala uma única vez,
                                  // passando a lista completa de docentes!
                                  final novaEscala = EscalaModel(
                                    id: '', // O repositório ou Firebase deve gerar o ID
                                    idCurso: curso.nome,
                                    turma: turma.nome,
                                    nomeUnidadeCurricular: uc,
                                    idDocentes: docentes
                                        .toList(), // Convertemos o Set de professores para List
                                    diaDaSemana: dia,
                                    blocoTurno: turno,
                                    sala: 'A Definir', // Valor padrão da sala
                                  );

                                  await repo.criarEscala(novaEscala);
