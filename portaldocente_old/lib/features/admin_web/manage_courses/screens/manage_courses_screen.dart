            ...
            child: Card(
              ...
              child: Padding(
                ...
                child: cursosAsync.when(
                  ...
                  data: (cursos) {
                    if (cursos.isEmpty) {
                      return const Center(child: Text('Nenhum curso cadastrado ainda. Clique em "Novo Curso" acima!'));
                    }
                          ...
                          trailing: Row(
                            ...
                            children: [
                              IconButton(
                                ...
                                onPressed: () async {
                                  final cursoEditado = await showDialog<CursoModel>(
                                    context: context,
                                    builder: (context) => CursoFormDialog(cursoExistente: curso),
                                  );
                                  if (cursoEditado != null) {
                                    await ref.read(cursoRepositoryProvider).updateCurso(cursoEditado);
                                    ref.invalidate(cursosListProvider);
                                  }
                                },
                              ),
                                ...
                                onPressed: () async {
                                  final ok = await showDialog<bool>(
                                    ...
                                    builder: (ctx) => AlertDialog(
                                      ...
                                      content: Text('Deseja excluir o curso ${curso.nome}?'),
                                      actions: [
                                  ...
                                  if (ok == true) {
                                    await ref.read(cursoRepositoryProvider).deleteCurso(curso.id);
                                    ref.invalidate(cursosListProvider);
                                  }
                                },
                              ),
                                  ...
                                  Consumer(
                                    builder: (context, ref, child) {
                                      ...
                                      return ucsAsync.when(
                                        loading: () => const LinearProgressIndicator(),
                                        error: (err, stack) => Text('Erro ao carregar UCs: $err'),
                                        data: (ucs) {
                                          if (ucs.isEmpty) {
                                            return const Padding(
                                              padding: EdgeInsets.symmetric(vertical: 10),
                                              child: Text('Nenhuma UC cadastrada ainda.', style: TextStyle(color: Colors.grey, fontSize: 13)),
                                            );
                                          }
                                          return Column(
                                            children: ucs.map((uc) {
